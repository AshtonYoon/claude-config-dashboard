...
Usage: python3 ~/.claude/tools/dashboard.py [--port 9876]
"""
...
def collect_plugins(settings: dict) -> list:
...
def collect_mcp_servers(settings: dict) -> list:
    servers = []
    seen: set = set()
...
def _tab_btns() -> str:
    tabs = [("plugins","Plugins"),("agents","Agents"),("skills","Skills"),
            ("commands","Commands"),("hooks","Hooks"),("mcp","MCP Servers"),("rules","Rules")]
    return "".join(
...
def render_plugins(plugins: list) -> str:
    ...
    for p in plugins:
        ...
        title = _open_link(f'<span class="font-semibold text-gray-900">{name}</span>', rp) if rp else f'<span class="font-semibold text-gray-900">{name}</span>'
...
def render_agents(agents: list) -> str:
    ...
    for cat, items in sorted(cats.items()):
        rows = "".join(
            ...
            f'<td class="px-4 py-2 whitespace-nowrap">{_open_link(_e(a["name"]), a["path"], "font-medium text-indigo-700")}</td>'
...
def render_skills(skills: list) -> str:
    ...
    for s in skills:
        ...
        if src == "custom": badge = '<span class="badge source-custom">custom</span>'
        elif "plugin" in src: badge = f'<span class="badge source-plugin">{_e(src.replace("plugin:",""))}</span>'
        else: badge = f'<span class="badge source-plugin">{_e(src)}</span>'
        if is_sym: badge += ' <span class="badge source-symlink">symlink</span>'
        title = _open_link(f'<span class="font-medium text-sm text-indigo-700">{name}</span>', path) if path else f'<span class="font-medium text-sm text-gray-900">{name}</span>'
        cards.append(f"""<div class="card">
  <div class="flex items-start justify-between mb-1">{title}<div class="flex gap-1 ml-2">{badge}</div></div>
  {f'<p class="text-xs text-gray-500">{desc}</p>' if desc else ''}
</div>""")
    return "".join(cards)
...
def render_commands(commands: list) -> str:
    ...
    for c in commands:
        ...
        rows.append(f'<tr class="border-b hover:bg-gray-50"><td class="px-4 py-2 whitespace-nowrap">{link}</td><td class="px-4 py-2 text-gray-600 text-sm">{desc}</td></tr>')
    return "".join(rows)

def render_hooks(hooks: list) -> str:
    colors = {
        "PreToolUse":"bg-yellow-100 text-yellow-800","PostToolUse":"bg-blue-100 text-blue-800",
        "Stop":"bg-red-100 text-red-700","SubagentStop":"bg-orange-100 text-orange-700",
        "UserPromptSubmit":"bg-green-100 text-green-800","PreCompact":"bg-purple-100 text-purple-700",
    ...
    for h in hooks:
        ...
        cmd_html = _open_link(f'<code class="text-xs text-gray-700 break-all">{cmd_display}</code>', h["path"]) if h["path"] else f'<code class="text-xs text-gray-700 break-all">{cmd_display}</code>'
...
def render_mcp(servers: list) -> str:
    ...
    for s in servers:
        ...
        if len(s.get("args",[])) > 4: args += " ..."
        src = _e(s.get("source",""))
        cards.append(f"""<div class="card">
  <div class="flex items-center justify-between mb-1">
    <h3 class="font-semibold text-gray-900">{_e(s["name"])}</h3>
    {f'<span class="badge bg-gray-100 text-gray-500">{src}</span>' if src else ''}
  </div>
  <code class="text-xs text-gray-600 break-all">{_e(s.get("command",""))} {args}</code>
</div>""")
    return "".join(cards)
...
<title>Claude Config Dashboard</title>
...
<div class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
  <div class="flex items-center gap-3">
    ...
    <div>
      ...
      <p class="text-xs text-gray-400">~/.claude · {now}</p>
    </div>
  </div>
  <div class="flex gap-4 text-center">
    {"".join(f'<div><div class="text-2xl font-bold text-indigo-600">{n}</div><div class="text-xs text-gray-500">{label}</div></div>'
      for n, label in [(len(p),"Plugins"),(len(ag),"Agents"),(len(sk),"Skills"),(len(co),"Commands"),(len(ho),"Hooks"),(len(mc),"MCP")])}
  </div>
</div>

<div class="bg-white border-b border-gray-200 px-6">
  <div class="flex gap-1 py-2">{_tab_btns()}</div>
</div>
...
<div id="tab-agents" class="tab-content">
  <p class="text-xs text-gray-400 mb-4">{len(ag)}개 에이전트 · {n_cats}개 카테고리 · 이름 클릭 시 기본 앱으로 열기</p>
  {render_agents(ag)}
</div>

<div id="tab-skills" class="tab-content">
  <p class="text-xs text-gray-400 mb-3">이름 클릭 시 기본 앱으로 열기</p>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">{render_skills(sk)}</div>
</div>

<div id="tab-commands" class="tab-content">
  <p class="text-xs text-gray-400 mb-3">커맨드 클릭 시 기본 앱으로 열기</p>
...
<div id="tab-hooks" class="tab-content">
  <p class="text-xs text-gray-400 mb-3">커맨드 클릭 시 스크립트 파일 열기</p>
  <div class="space-y-3">{render_hooks(ho)}</div>
</div>

<div id="tab-mcp" class="tab-content">
  <div class="space-y-3">{render_mcp(mc)}</div>
</div>

<div id="tab-rules" class="tab-content">
  <p class="text-xs text-gray-400 mb-3">파일명 클릭 시 기본 앱으로 열기</p>
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if parsed.path == "/open":
                ...
                self._respond(200, self._html, "text/html; charset=utf-8")
...
def main():
    ...
    data = {
        ...
        "mcp_servers":collect_mcp_servers(settings),
        "rules":      collect_rules(),
    }
    ...
    server = HTTPServer(("localhost", args.port), make_handler(html))

    print(f"\nDashboard → {url}")
    print("파일 이름 클릭 시 기본 앱으로 열립니다. 종료: Ctrl+C\n")
